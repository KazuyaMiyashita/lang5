...
object Ast {
  ...
  final case class Program(definitions: List[FunctionDefinition])

  final case class FunctionDefinition(name: String, args: List[String], body: List[Expression])
...
//    IfExpression(condition, thenClause, None)
  ...
  def defFun(name: String, args: List[String], body: List[Expression]): FunctionDefinition =
    FunctionDefinition(name, args, body)

  def callFun(name: String, args: List[Expression]): FunctionCall = FunctionCall(name, args)

  def program(definitions: FunctionDefinition*): Program = Program(definitions.toList)
